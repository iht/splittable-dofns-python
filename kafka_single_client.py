...
def produce_messages(
    bootstrap_server: str, data_filename: str, topic_name: str, limit: int
):
    ...
    with open(data_filename, "r") as f:
        ...
        for line in f:
            producer.send(
                topic=topic_name, partition=k % NUM_PARTITIONS, value=line.encode()
            )
...
def read_messages(bootstrap_server: str, topic_name: str):
    peeker: KafkaConsumer = KafkaConsumer(
        topic_name, group_id="peeker-group", bootstrap_servers=[bootstrap_server]
    )
    ...
    while True:
        ...
        for consumer in consumers:
            ...
            last_seen_offset = consumer.committed(
                TopicPartition(topic=topic_name, partition=current_partition)
            )
...
def create_topic(bootstrap_server: str, topic_name: str):
    admin_client: KafkaAdminClient = KafkaAdminClient(
        bootstrap_servers=bootstrap_server, client_id="create-topic"
    )
