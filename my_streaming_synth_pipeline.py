...
from mydofns.synthetic_sdfn_streaming import (
    GeneratePartitionsDoFn,
    MyPartition,
    ProcessPartitionsSplittableDoFn,
)
...
def run_pipeline_streaming(beam_options):
    ...
    with beam.pipeline.Pipeline(options=pipeline_options) as p:
        partitions: PCollection[MyPartition] = (
            p | beam.Create([0]) | beam.ParDo(GeneratePartitionsDoFn())
        )
