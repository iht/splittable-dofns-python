...
class ReadPartitionsDoFn(beam.DoFn):
    def __init__(
        self, topic: str, bootstrap_server: str, *unused_args, **unused_kwargs
    ):
        self._topic = topic
    ...
    def setup(self):
        self._kafka_client = KafkaConsumer(
            self._topic,
...
class ProcessKafkaPartitionsDoFn(beam.DoFn, RestrictionProvider):
    ...
    def _create_and_get_consumer(self, partition):
        ...
        if tp in self._kafka_clients.keys():
            ...
            client = KafkaConsumer(
                group_id="my-beam-consumer-group",
                bootstrap_servers=[self._bootstrap],
    ...
    def initial_restriction(self, partition: int) -> OffsetRange:
        ...
        committed_offset = kafka_client.committed(
            TopicPartition(topic=self._topic, partition=partition)
        )
